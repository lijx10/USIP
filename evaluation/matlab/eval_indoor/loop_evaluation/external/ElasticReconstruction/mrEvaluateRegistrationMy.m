function [ recall, precision, inlierNum_mean, inlierRatio_mean ] = mrEvaluateRegistrationMy( result, gt, gt_info, err2 )
    if ~exist( 'err2', 'var' )
        err2 = 0.04;
    end
    num = gt( 1 ).info( 3 );
        ...
        if ( result( i ).info( 2 ) - result( i ).info( 1 ) > 1 )
            ...
            if idx == 0
                ...
                if ( p <= err2 )
                    ...
                    inlierNum = [inlierNum, result(i).inlierNum];
                    inlierRatio = [inlierRatio, result(i).inlierRatio];
                else
                    bad = bad + 1;
                end
    ...
    inlierNum_mean = mean(inlierNum);
    inlierRatio_mean = mean(inlierRatio);
    disp( [ 'recall : ' num2str( recall ) ' ( ' num2str( good ) ' / ' num2str( gt_num ) ' )' ] );
    disp( [ 'precision : ' num2str( precision ) ' ( ' num2str( good ) ' / ' num2str( rs_num ) ' )' ] );
    disp( [ 'inlier number : ', num2str(inlierNum_mean), ' inlier ratio : ', num2str(inlierRatio_mean) ] );
end
