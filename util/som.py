...
class BatchSOM():
    ...
    def query_topk(self, node, x, k):
        ...
        device = x.device
        node = node.to(x.device)
        node_idx_list = torch.from_numpy(np.arange(self.node_num).astype(np.int64)).to(device)  # node_num LongTensor

        # expand as BxCxNxnode_num
        node = node.unsqueeze(2).expand(x.size(0), x.size(1), x.size(2), self.rows * self.cols)
        x_expanded = x.unsqueeze(3).expand_as(node)
        ...
        node_idx_list = node_idx_list.unsqueeze(0).unsqueeze(0).unsqueeze(3).expand_as(min_idx_expanded).long()  # BxNxnode_numxk
        mask = torch.eq(min_idx_expanded, node_idx_list).int()  # BxNxnode_numxk
        # mask = torch.sum(mask, dim=3)  # BxNxnode_num
    ...
    def query(self, node, x):
        ...
        node_idx_list = node_idx_list.unsqueeze(0).unsqueeze(0).expand_as(min_idx_expanded).long()  # BxNxnode_num
