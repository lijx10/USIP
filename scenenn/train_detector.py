...
from scenenn.options_detector import Options
...
from data.scenenn_detector_loader import SceneNNDetectorLoader
...
if __name__=='__main__':
    ...
    valset = SceneNNDetectorLoader(opt.dataroot, 'val', opt)
    combine_set = trainset + valset

    dataset_size = len(combine_set)
    trainloader = torch.utils.data.DataLoader(combine_set, batch_size=opt.batch_size, shuffle=True,
                                              num_workers=opt.nThreads, drop_last=True, pin_memory=True)
    print('#training point clouds = %d' % len(combine_set))
    ...
    for epoch in range(501):
        ...
        for i, data in enumerate(trainloader):
            ...
            if i % int(32 / opt.batch_size * 20) == 0 and i > 0:
                # print/plot errors
        ...
        if epoch >= 0 and epoch%1==0:
            ...
            # save models
            if (model.test_loss_average.item() <= best_loss + 1e-5) and (model.test_chamfer_pure_average.item() < 0.85) and (epoch>2*lr_decay_step):
